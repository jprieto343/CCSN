...
#   Getting the expexted value of the snr
    ...
    expected = 4*np.sqrt(deltaf*(yfa/psd))  
    ...
    if grafica == 1:
        ...
        f, Pxx_den = signal.welch(xs, fs,nperseg=8192)
        pylab.figure(figsize=[15, 5])
        pylab.title('Power Spectral Density of strain ')
        pylab.loglog(f, np.sqrt(Pxx_den),label='WGN PSD')
        pylab.loglog(xf,np.sqrt(yfa),label='FFT of template')
    ...
    return maximo,np.sqrt(yfa), np.sqrt(Pxx_den),f,xf